...
# title           :confirm_update.py
# description     :ConfirmUpdate menu definition.
# author          :Eric Kimsey
# usage           :
# notes           :
# python_version  :3.5+
...
class ConfirmUpdate(OptionSelect):
    TITLE = 'Update movie in LibraryDB'

    def __init__(self, movie: LibraryDBMovie):
        """
        Create ConfirmUpdate selection menu.
        :param LibraryDBMovie movie: LibraryDBMovie to be updated in LibraryDB
        """
        prompt = 'Update in LibraryDB? '
        options = {
            'Y': 'Yes',
            'N': 'No'
        }
    ...
    def process_input(self, entry: str) -> tuple:
        ...
        if entry.upper() == 'B' or entry.upper() == 'N':
            ...
            result = librarydb.update_movie(self._movie)
            if result[0]:
                # Success
                title = globals.VERSION + ' - Update Successful'
                prompt = 'Press [Enter] to return to the main menu...'
                message = 'Movie was successfully updated in LibraryDB!'
